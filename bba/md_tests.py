...
def main():
    ...
    if corr_test:
        ...
        corrector_amplitude_scan(algorithm, element_list[0], method, axis, repeats, False, corr_amp_list)
    ...
    if combination_test:
        ...
        cycles_list = [1, 10, 20]
        combination(algorithm, element_list[0], method, axis, repeats, False, cycles_list, corr_amps)



def repeat_test(algorithm, element, method, axis, repeats, apply, quadrupole_scalar_=0.01, corrector_scalar_=1, cycles_=1, frequency_=8, decimated_=False):
...
def frequency_scan(algorithm, element, method, axis, repeats, apply, frequency_list):
    ...
    for frequency in frequency_list:
        frequency_values, frequency_errors = repeat_test(algorithm, element, method, axis, repeats, apply, frequency_=frequency)
    ...
    np.savetxt(f"frequency_scan_repeats_{repeats}_len_{len(frequency_list)}.csv", matrix, delimiter=",")


def corrector_amplitude_scan(algorithm, element, method, axis, repeats, apply, corr_amplitude_list):
    ...
    for amplitude in corr_amplitude_list:
        amp_values, amp_errors = repeat_test(algorithm, element, method, axis, repeats, apply, corrector_scalar_=amplitude)
    ...
    np.savetxt(f"corr_amp_scan_repeats_{repeats}_len_{len(corr_amplitude_list)}.csv", matrix, delimiter=",")
...
def honing_simple(algorithm, element, method, axis, repeats, quadrupole_scalar=0.01, corrector_scalar=1, cycles=1, frequency=8):
    ...
    value_list, error_list = repeat_test(algorithm, element, method, axis, repeats, apply, quadrupole_scalar_=quadrupole_scalar, corrector_scalar_=corrector_scalar, cycles_=cycles, frequency_=frequency) # Can add other args here.
    matrix = np.zeros(shape=(2, repeats))
    matrix[0, :] = value_list
    matrix[1, :] = error_list
    np.savetxt(f"honing_simple_repeats_{repeats}_c{cycles}_f{frequency}_q{quadrupole_scalar}_cs{corrector_scalar}.csv", matrix, delimiter=",")
...
def combination(algorithm, element, method, axis, repeats, apply, cycle_list, corr_amplitude_list):
    ...
    for cycle in cycle_list:
        for corr_amp in corr_amplitude_list:
            value_list, error_list = repeat_test(algorithm, element, method, axis, repeats, apply, corrector_scalar_=corr_amp, cycles_=cycle)
    ...
    for cycle in cycle_list:
        ...
        np.savetxt(f"combination_scan_repeats_{repeats}_cycle_{cycle}.csv", matrix, delimiter=",")
