...
class SBBA(Algorithm):
    ...
    def run(self, element, plane_info, max_orbit) -> RawData:
        ...
        for quad in quad_list:
            ...
            corr_step = self._accelerator.microrads(corrector, plane_info) * self.corrector_scalar
            ...
            quad_pv_root = self._accelerator.element_to_pv_prefix(quad).replace("-", "_")

            for index, step in enumerate(corrector_step_list):
                log.info(f"Index: {index}, Step: {step}")
                # Step the corrector to the value.
                self._accelerator.set_corrector(corrector, plane_info, corrector_sp + step)
