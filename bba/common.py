...
class Algorithm(ABC):
    ...
    def check_beam_current(self, initial_current) -> bool:
        ...
        if current_drop > MAXIMUM_CURRENT_DROP:
            ...
            raise LowBeamCurrent(f"Beam current dropped by >{MAXIMUM_CURRENT_DROP}mA. Cancelling BBA.")
        ...
        if current_drop > MINIMUM_CURRENT_DROP:
            ...
            while True:
                ...
                if response == "n":
                    ...
                    raise LowBeamCurrent(f"Beam current dropped by {MINIMUM_CURRENT_DROP}-{MAXIMUM_CURRENT_DROP}mA. User cancelled BBA.")
                elif response == "y":
                    current_drop = initial_current - self._accelerator.get_beam_current()
                    if current_drop < MINIMUM_CURRENT_DROP:
                        break
