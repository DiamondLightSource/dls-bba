...
def main():
    ...
    if cycle_test:
        ...
        data = np.genfromtxt(
            f"cycle_scan_repeats_{repeats}_len_{len(cycle_list)}.csv", delimiter=","
        )
        y = []
        y_err = []
        ...
        plt.savefig(
            f"cycle_scan_repeats_{repeats}_len_{len(cycle_list)}_plot.png",
            bbox_inches="tight",
            dpi=1200,
        )
        plt.show()
    ...
    if quad_test:
        ...
        data = np.genfromtxt(
            f"quad_amp_scan_repeats_{repeats}_len_{len(quad_amp_list)}.csv",
            delimiter=",",
        )
        y = []
        y_err = []
        ...
        plt.grid(which="both", axis="both")
        plt.savefig(
            f"quad_amp_scan_repeats_{repeats}_len_{len(quad_amp_list)}_plot.png",
            bbox_inches="tight",
            dpi=1200,
        )
        plt.show()
    ...
    if honing_test:
        ...
        # y_err = []
        plt.errorbar(
            x_axis, y, marker=".", capsize=5, label=f"{method}, Offset:{offset}"
        )
        ...
        method = "FBBA"
        data = np.genfromtxt(
            f"{TEMP_FILEPATH_ROOT}/honing_simple_repeats_{method}_{repeats}_c1_f8_q{quadrupole_scalar}_cs{corrector_scalar}_fft{_fft}_offset{offset}.csv",
            delimiter=",",
        )
        o0_f = data[index, :]
        y = offset_0_original
        ...
        plt.errorbar(
            ...
            marker=".",
            capsize=5,
            label=f"{method}, Offset:{offset}, fft:{_fft}",
        )
        ...
        method = "FBBA"
        data = np.genfromtxt(
            f"{TEMP_FILEPATH_ROOT}/honing_simple_repeats_{method}_{repeats}_c16_f8_q{quadrupole_scalar}_cs{corrector_scalar}_fft{_fft}_offset{offset}.csv",
            delimiter=",",
        )
        o0_t = data[index, :]
        y = offset_0_original
        ...
        method = "FBBA"
        data = np.genfromtxt(
            f"{TEMP_FILEPATH_ROOT}/honing_simple_repeats_{method}_{repeats}_c10_f50_q{quadrupole_scalar}_cs{corrector_scalar}_fft{_fft}_offset{offset}.csv",
            delimiter=",",
        )
        o100_t = data[index, :]
        y = offset_100_original
        ...
        plt.legend()
        plt.savefig(
            f"{TEMP_FILEPATH_ROOT}/honing_simple_repeats_change_plot.png",
            bbox_inches="tight",
            dpi=1200,
        )
        plt.show()
