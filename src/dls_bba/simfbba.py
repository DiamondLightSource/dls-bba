...
class SimFastBBA(Algorithm):
    ...
    def run(self, components_pair: list[Components]) -> RawData:
        ...
        for quadrupole, quad_name in zip(
            ...
            hcorr_kick = self.machine.corrector_kick(components_pair[0].corrector)
            hcorr_sp = self.machine.get_corrector_setpoint(components_pair[0])

            vcorr_kick = self.machine.corrector_kick(components_pair[1].corrector)
            vcorr_sp = self.machine.get_corrector_setpoint(components_pair[1])
            ...
            for index, axis in enumerate(["x", "y"]):
                ...
                excitations[f"Low_{axis}"] = Excitation(
                    self.machine, components_pair[index], oscillations[axis], low_start
                )
