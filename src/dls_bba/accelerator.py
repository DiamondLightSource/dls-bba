...
MASTER_CALIBRATION_PATH = (
    "/dls_sw/work/common/matlab/mml/machine-new/diamond/master_calibration.csv"
)
...
BPM_TO_QUAD_SPECIAL = {
    ...
    "SR10C-DI-EBPM-02": [
        "SR10A-PC-Q2B-02",
        "SR10A-PC-Q3B-03",
    ],  # Cell 10, magnet length inconsistency.
...
class Accelerator:
    ...
    def __init__(self, ringmode=None):
        ...
        self.bpm_h_fofb_enabled = self.lattice.get_element_values(
            "BPM", "x_fofb_disabled", pytac.RB
        )
        self.bpm_v_fofb_enabled = self.lattice.get_element_values(
    ...
    def measure_bpms(self, plane_info):
        ...
        for attempt in range(1, TRIES + 1):
            try:
                bpm_values = self.lattice.get_element_values(
                    "BPM", plane_info.axis.lower()
                )
                ...
                if attempt == TRIES:
                    ...
                    raise AcceleratorException(
                        f"Failed to retrieve bpm values {TRIES} times:\n{e}"
                    )
                cothread.Sleep(1)
            else:
                break
    ...
    def special_correctors(self, plane):
        ...
        if plane.corrector == "HSTR":
            corrector_pv_roots = [
                self.element_to_pv_prefix(corrector_pv_root, plane)
                for corrector_pv_root in self.hstrs
            ]
        elif plane.corrector == "VSTR":
            corrector_pv_roots = [
                self.element_to_pv_prefix(corrector_pv_root, plane)
                for corrector_pv_root in self.vstrs
            ]
    ...
    def bpm_to_quad(self, bpm):
        ...
        if bpm_pv_prefix not in BPM_TO_QUAD_SPECIAL:
            quads_keys = [
                quad_key
                for quad_key, bpm_value in self.quad_to_bpm_dict.items()
                if bpm_value == bpm_pv_prefix
            ]
            quads_list = []
    ...
    def microrads(self, corrector, plane) -> float:
        ...
        gradient = (float(final_current) - float(initial_current)) / (
            float(final_rad) - float(initial_rad)
        )
