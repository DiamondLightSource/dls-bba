...
class Ticker:
    ...
    def __do_tick(self) -> None:
        ...
        action = "run"
        while action != "stop" and fraction > 0:
            if self.__action:
                action = self.__action.Wait()

            if action == "run":
                fraction = self.__worker.work()
                self.__progress(fraction)

        self.__set_state("Complete")
        self.__worker.finish(self.__manual_stop)
