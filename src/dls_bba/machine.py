    ...
    def zero_origins(self, folder_path: str) -> None:
        ...
        for bpm_name in self.bpms_names:
            for axis in ["x", "y"]:
                ...
                bcd_names.append(bcd_pv)
                golden_names.append(golden_pv)

        golden_vals = caget(golden_names)
        for name, offset in zip(golden_names, golden_vals, strict=True):
        ...
        caput(bcd_names, 0, wait=True)
        caput(golden_names, 0, wait=True)
        Sleep(0.2)
